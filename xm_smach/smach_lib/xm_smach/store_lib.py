...
class Get_object(State):
    def __init__(self):
        State.__init__(self, outcomes = ['succeeded','finish','error'],
                        ...
                        output_keys = ['current_obj','current_task'])
    ...
    def execute(self , userdata):
        try:
            ...
            rospy.logwarn(userdata.object_list)
            rospy.logwarn(userdata.current_task)
            if len(userdata.object_list) > userdata.current_task:
...
class Get_position(State):
    ...
    def execute(self,userdata):
        try:
            ...
            userdata.position_list[mode].point.x += 0.100
            return 'succeeded'
