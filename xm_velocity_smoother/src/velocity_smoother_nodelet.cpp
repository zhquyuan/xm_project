...
void VelocitySmoother::velocityCB(const geometry_msgs::Twist::ConstPtr& msg)
{
  ...
  // Bound speed with the maximum values(绝对值)肖：？？这里是最小绝对值吧
  target_vel.linear.x  =
...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
        ...
        ((ros::Time::now() - last_cb_time).toSec() > std::min(3.0*cb_avg_time, 0.5)))//超时,丢失了3个msgs，或者不是丢包了，而是根本没有需要平滑的速度发下来
    {
      ...
      input_active = false;
      if (IS_ZERO_VEOCITY(target_vel) == false)//肖：丢失了3个包时，把目标速度置为0
      {
    ...
    {//肖：当目标速度置为0已有一段时间，但上次command速度与当前速度差值超过0.2时，不再相信上次command速度，将其调整为直接从机器人反馈回来的速度
      ...
      // be proportional to max v and w...肖：当前速度与上次command速度之间差值可以调整下
        ...
        (target_vel.angular.z != last_cmd_vel.angular.z))
    {
      ...
      double theta3 = atan2(Bw, Bv) - atan2(Aw, Av);//xiao注释于7.25，这个不适合于三轮全向轮
      ...
      // }
      //上面两步实际上已经产生溢出了，gg
      if (theta3 < 0)//xiao这里似乎有点问题，有时间就修正
      {
        // overconstrain linear velocity
        max_v_inc = (max_w_inc*std::abs(v_inc))/std::abs(w_inc);
        
        //max_u_inc = (max_w_inc*std::abs(u_inc))/std::abs(w_inc);//xiao我乱改的
      }
      else
      {
        // overconstrain angular velocity
